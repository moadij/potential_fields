...
def computeTrajectory(robotPos, goalPos, distanceSensors):
	...
	d = sqrt(((goalPos[0]- robotPos[0])**2) + ((goalPos[1]- robotPos[1])**2))
	
	theta = np.arctan2((goalPos[1]-robotPos[1]) / (goalPos[0]-robotPos[0]))
	delta_x = alpha_1 * s_1 * np.cos(theta)
	delta_y = alpha_1 * s_1* np.sin(theta)
	trajectory = [delta_x,delta_y]
	
	#obstcle calculations

	for i in range(16):
		if distanceSensors[i] > 3:
    		#find the opposite vector
			#calculate the direction
			direction_of_sensor = i * 22.5 +robotPos[3]
			direction_away_obst = direction_of_sensor + 180
			ratio = np.arctan(direction_away_obst)
			new_direction = [1, ratio]
			normalize(new_direction)

	new_trajectory = [trajectory[0] + new_direction[0], trajectory[1] + new_direction[1]]

	# call this function to normalize the trajectory vector (so that it is a unit vector)
	new_trajectory = normalize(new_trajectory)
